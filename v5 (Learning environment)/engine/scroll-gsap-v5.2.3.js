// ============================================================
// ENGINE START — scroll-gsap.js v5.2.3
...
(function () {
  ...
  //   waypoints: [0.45, 0.60],
  //
  //   // [v5.2.3] Scroll speed governor + video duration
  //   scrollSpeed:   0.5,   // 0.1 (crawl) → 1.0 (native). Default 0.5.
  //   videoDuration: 65.7,  // actual video length in seconds (fixes early stop)
  //
  //   videoMap: [ ... ],
  //   scenes:   [ ... ]
  // };
  ...
  function mapToVideoTime(progress) {
    ...
    if (progress >= last.scroll) {
      // Extrapolate linearly from last anchor to cfg.videoDuration if set
      const fullDuration = cfg.videoDuration || last.time;
      if (cfg.videoDuration && cfg.videoDuration > last.time && last.scroll < 1) {
  ...
  // ── Scroll speed governor [v5.2.3] ────────────────────────────────────
  // Caps wheel delta inside the hero zone so fast scrolling can't make the
  // video unwatchable. Trackpad micro-events (<TRACKPAD_THRESHOLD px) are
  // passed through unchanged — this only throttles large mouse-wheel ticks.
  // cfg.scrollSpeed: 0.1 (crawl) → 1.0 (native speed). Default: 0.5
  function attachScrollGovernor() {
    const speed    = cfg.scrollSpeed !== undefined ? cfg.scrollSpeed : 0.5;
    const maxDelta = Math.round(100 * speed);  // px per wheel tick
    const TRACKPAD = 30;                        // below this → trackpad passthrough

    window.addEventListener('wheel', function(e) {
      if (!st) return;
      // st.end is the scrollY at progress=1.0
      if (window.scrollY > st.end) return;        // below hero — native scroll
      var absD = Math.abs(e.deltaY);
      if (absD <= TRACKPAD) return;               // trackpad micro-delta
      e.preventDefault();
      var governed = Math.sign(e.deltaY) * Math.min(absD, maxDelta);
      window.scrollBy({ top: governed, behavior: 'auto' });
    }, { passive: false });
  }

  // ── Start ─────────────────────────────────────────────────────────────────
  function startWithGovernor(initFn) {
    initFn();
    attachScrollGovernor();
  }

  if (useFrames) {
    preloadFrames(() => startWithGovernor(initScrollTrigger));
  } else {
    initVideoFallback(() => startWithGovernor(initScrollTrigger));
  }

})();
// ENGINE END — scroll-gsap.js v5.2.3
// ============================================================
